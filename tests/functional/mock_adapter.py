...
responses = {
    'GET': {
        ...
        '/accounts/1?ws.op=findSubscribers&email=joe%40example.com': (
            {}, 'accounts/findSubscribers'),
        '/accounts/1?ws.op=findSubscribers&email=joe%40example.com&'
        'ws.show=total_size': ({}, 'accounts/findSubscribers_ts'),
        '/accounts/1?ws.op=getWebForms': ({}, 'accounts/webForms'),
        '/accounts/1?ws.op=getWebFormSplitTests': (
            {}, 'accounts/webFormSplitTests'),
        ...
        '/accounts/1/lists/303449/subscribers/1?ws.op=getActivity'
        '&ws.show=total_size': ({}, 'subscribers/get_activity_ts'),
        ...
        '/accounts/1/lists/303449/subscribers?ws.op=find&'
        'email=joe%40example.com&ws.show=total_size': (
            {}, 'subscribers/find_ts'),
    },
    'POST': {
        '/accounts/1/lists/303449/any_collection':  ({
            'status': '201',
            'location': '/accounts/1/lists/303449/any_collection/1',
        },
            None,
        ),
...
def request(self, url, method, **kwargs):
    ...
    (headers, file_name) = responses[method][url]
    if 'status' not in headers:
        # assume 200 OK if not otherwise specified
        headers['status'] = '200'
    if file_name is None:
        return (headers, '')
    path = sep.join(__file__.split(sep)[:-2] + ['data', ''])
    filename = "{0}{1}.json".format(path, file_name)
...
class MockAdapter(OAuthAdapter):
    ...
    @patch('oauth2.Client.request', request)
